...
class MazeImage:
    ...
    def __init__(self, maze, height, width):
        ...
        self.writer = cv2.VideoWriter('q_learning.mp4', cv2.VideoWriter_fourcc('m', 'p', '4', 'v'), 30, (self.map_org.shape[1], self.map_org.shape[0]))

    def show(self, agent, trial):
        self.map_now = self.map_org.copy()
        _y, _x = agent.state
        center = (int((_x + 0.5) * self.xstride), int((_y + 0.5) * self.ystride))
        cv2.putText(self.map_now, "trial: "+str(trial+1), (self.height-self.xstride*9/10, self.width-self.ystride/2),cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255))
        cv2.circle(self.map_now, center, 11, (255, 255, 255), -1, 16)
